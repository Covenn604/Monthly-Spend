...
function renderAccountBalances(){
 $('#overview-balances').innerHTML=state.accounts.length?state.accounts.map(a=>`<article class="balance-card"><h3>${esc(a.name)}</h3><strong class="balance-value ${a.balance<0?'up':''}">${moneyHtml(a.balance)}</strong><dl><div><dt>Opening balance</dt><dd>${moneyHtml(a.opening)}</dd></div><div><dt>Recorded activity</dt><dd>${moneyHtml(a.balance-a.opening)}</dd></div></dl></article>`).join(''):'<p class="empty">No accounts yet. Add an account in Accounts & categories to see its balance here.</p>';
}
